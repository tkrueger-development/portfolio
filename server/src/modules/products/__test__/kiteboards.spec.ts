...
describe('GET /api/products/kiteboards/:count', () => {
  ...
  describe('/api/products/kiteboards/5', () => {
    let body:     unknown;
    let response: request.Response;

    beforeAll(async () => {
      response = await request(app).post('/api/products/kiteboards/5').send(filter);
      body = response.body;
    });
    ...
    it('is an array of ProductOverviewDTOs', () => {
      ...
      (body as Array<any>).forEach((product: any) => {
        ...
        expect(product.category)                                    .equals('kiteboard');
  ...
  describe('GET /api/products/kiteboards/0', () => {
    it('clamps product @count to minimum of 1', async () => {
      const { body } = await request(app).post('/api/products/kiteboards/0');

      expect(body.length).equals(1);
    });
  });

  describe('GET /api/products/kiteboards/11', () => {
    it('clamps product @count to a maximum of 20', async () => {
      const { body } = await request(app).post('/api/products/kiteboards/21');

      expect(body.length).equals(20);
    });
  });

  describe('GET /api/products/kiteboards/xyz', () => {
    it('responds with 400 when @count is not a parsable number', async () => {
      await request(app).post('/api/products/kiteboards/xyz').expect(400);
    });
  });

  describe('GET /api/products/kiteboards/', () => {
    it('responds with 404 when @count is undefined', async () => {
      await request(app).post('/api/products/kiteboards/').expect(404);
    });
  });
});