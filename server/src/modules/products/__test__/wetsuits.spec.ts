...
describe('GET /api/products/wetsuits/:count', () => {
  ...
  describe('/api/products/wetsuit/5', () => {
    let body:     unknown;
    let response: request.Response;

    beforeAll(async () => {
      response = await request(app).post('/api/products/wetsuits/5').send(filter);
      body = response.body;
    });
    ...
    it('is an array of ProductOverviewDTOs', () => {
      ...
      (body as Array<any>).forEach((product: any) => {
        ...
        expect(product.category)                                    .equals('wetsuit');
  ...
  describe('GET /api/products/wetsuits/0', () => {
    it('clamps product @count to minimum of 1', async () => {
      const { body } = await request(app).post('/api/products/wetsuits/0');

      expect(body.length).equals(1);
    });
  });

  describe('GET /api/products/wetsuits/11', () => {
    it('clamps product @count to a maximum of 20', async () => {
      const { body } = await request(app).post('/api/products/wetsuits/21');

      expect(body.length).equals(20);
    });
  });

  describe('GET /api/products/wetsuits/xyz', () => {
    it('responds with 400 when @count is not a parsable number', async () => {
      await request(app).post('/api/products/wetsuits/xyz').expect(400);
    });
  });

  describe('GET /api/products/wetsuits/', () => {
    it('responds with 404 when @count is undefined', async () => {
      await request(app).post('/api/products/wetsuits/').expect(404);
    });
  });
});