...
import { fetchWetsuits } from '../api/wind-and-wave';

import { useAppDispatch, useAppSelector } from '../state/hooks';
import { toggleBrandFilter, toggleSizeFilter, setPriceFilter, setRatingAverageFilter, resetFilters } from '../state/features/wetsuit-filter-slice';
...
interface AllPossibleWetsuitFilters {
  brand?:         Array<string>;
  size?:          Array<string>;
  price?:         [number, number];
...
const WetsuitsCategory = () => {
  const [allPossibleFilters, setAllPossibleFilters] = useState<AllPossibleWetsuitFilters>({ });

  const filterState = useAppSelector((state) => state.wetsuitFilters);
  const dispatch = useAppDispatch();

  const { data: wetsuits, isError, error, isLoading, refetch } = useQuery(
    ['wetsuit'], 
    () => fetchWetsuits({ filters: createFilterDTO({ filters: filterState }) }), {
