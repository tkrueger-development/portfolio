...
import { fetchBars } from '../api/wind-and-wave';

import { useAppDispatch, useAppSelector } from '../state/hooks';
import { toggleBrandFilter, setPriceFilter, resetFilters, setRatingAverageFilter } from '../state/features/bar-filter-slice';
...
const BarsCategory = () => {
  const [allPossibleFilters, setAllPossibleFilters] = useState<Partial<Omit<ProductFilterOptions, 'size'>>>({ });

  const filterState = useAppSelector((state) => state.barFilters);
  const dispatch = useAppDispatch();

  const { data: bars, isError, error, isLoading, refetch } = useQuery(
    ['bars'], 
    () => fetchBars({ filters: createFilterDTO({ filters: filterState }) }), {
